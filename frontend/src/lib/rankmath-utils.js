...
export async function fetchRankMathMetadata(slug, parent) {
  try {
    ...
    let fullUrl;
    if (parentPath) {
      fullUrl = `${process.env.WP_URL}/${parentPath}/${slug}/`;
    } else {
      fullUrl = `${process.env.WP_URL}/${slug}/`;
    }
   
    const apiUrl = `${process.env.WP_URL}/wp-json/rankmath/v1/getHead?url=${encodeURIComponent(fullUrl)}`;
